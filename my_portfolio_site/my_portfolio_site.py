...
from my_portfolio_site.pages import about, case_study, contact, services  # noqa: F401
...
def problems_section():
    ...
    return rx.box(
        rx.vstack(
            rx.heading("No more lost data.", size=FontSizes.SECTION_HEADER, color=Colors.TEXT),
            align="center",
        ),
        rx.box(
            *[
            rx.hstack(
                rx.text(icon, font_size=FontSizes.MAIN_TEXT),
                rx.vstack(
                    rx.text(title, font_weight="bold", font_size=FontSizes.MAIN_TEXT, color=Colors.TEXT),
                    rx.text(desc, font_size=FontSizes.SMALL_TEXT, color=Colors.TEXT),
                ),
                padding="1em",
                padding_left="8em",
            )
...
def how_it_works_section():
    ...
    return rx.box(
        rx.vstack(
            rx.heading("How it works", size=FontSizes.SECTION_HEADER, color=Colors.TEXT),
            align="center",
        ),
        rx.box(
            *[
            rx.hstack(
                rx.vstack(
                    rx.heading(phase, size=FontSizes.SUBSECTION_HEADER, color_scheme="teal",),
                    rx.text(desc, font_size=FontSizes.SMALL_TEXT, color=Colors.TEXT),
                ),
                padding="2em",
                padding_left="8em",
...
def why_us_section():
    return rx.box(
        rx.vstack(rx.heading("Why Choose This Service", size=FontSizes.SECTION_HEADER, color=Colors.TEXT), align="center"),
        rx.vstack(
            rx.list(
                rx.list.item("✔️ Keeps your existing lab workflows — no need to replace notebooks or protocols.", color=Colors.TEXT),
                rx.list.item("✔️ Delivers immediate value — searchable, organized data from day one.", color=Colors.TEXT),
                rx.list.item("✔️ Lab-agnostic and LLM-agnostic — works with any dataset, any LLM.  ", color=Colors.TEXT),
                rx.list.item("✔️ Scalable: start small (diary) → expand to full archive.  ", color=Colors.TEXT),
                rx.list.item("✔️ Built for scientists, by someone who understands lab pain points.", color=Colors.TEXT),
                font_size=FontSizes.MAIN_TEXT,
                spacing="5",
            ),
